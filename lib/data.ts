...
import { getCollectionPage, getWantlistPage, processWantlist as processWantlistWithApi } from './discogs';
import type { CollectionRelease, ProcessedWantlistItem, Pagination, DiscogsUser } from './types';
...
export async function getHeaderData() {
    ...
    const collectionPromise = getCollectionPage(user.username, token, 1, 1, 'added', 'desc');
    const wantlistPromise = getWantlistPage(user.username, token, 1, 1, 'added', 'desc');

    const [collectionResponse, wantlistResponse] = await Promise.all([collectionPromise, wantlistPromise]);
...
async function getFullCollection(user: DiscogsUser, token: string): Promise<CollectionRelease[]> {
    let allReleases: CollectionRelease[] = [];
    let page = 1;
    let totalPages = 1;

    do {
        const { data, pagination } = await getCollectionPage(user.username, token, page, 100, 'added', 'desc');
...
export async function getInitialCollection(): Promise<{ data: CollectionRelease[], pagination: Pagination }> {
    const { user, token } = await getAuthenticatedUser();
...
export async function getInitialWantlist(): Promise<{ data: ProcessedWantlistItem[], pagination: Pagination }> {
    const { user, token } = await getAuthenticatedUser();
    console.log(`[Data] Fetching initial wantlist page for ${user.username}`);
    
    const { data: rawWantlist, pagination } = await getWantlistPage(user.username, token, 1, ITEMS_PER_PAGE, 'added', 'desc');
